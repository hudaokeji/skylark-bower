...
Manager.prototype.install = function(json) {
    ...
    return Q.nfcall(mkdirp, componentsDir)
        .then(function() {
            ...
            mout.object.forOwn(that._dissected, function(decEndpoint, name) {
                ...
                if (exists && !that._installed[name] && !that._config.force) {
                    ...
                    // this name, but it was not installed by spkgm.
                    that._logger.warn(
                        'skipped',
                        name +
                            ' was not installed because there is already a non-spkgm directory with that name in the components directory (' +
                ...
                promise = Q.nfcall(rimraf, dst)
                    ...
                    .then(function() {
                        var metaFile = path.join(dst, '.spkgm.json');

                        decEndpoint.canonicalDir = dst;

                        // Store additional metadata in spkgm.json
        ...
        .then(function() {
            ...
            // See: https://github.com/spkgm/spkgm/issues/879
            mout.object.forOwn(
                that._dissected,
                function(pkg) {
                    // Sync dependencies
            ...
            function() {
                ...
                mout.object.forOwn(
                    ...
                    function(resolution, name) {
                        ...
                        // "resolution" entry in spkgm.json are deemed unnecessary
                        //
                        // Probably in future we want to use them for force given resolution, but for now...
                        // See: https://github.com/spkgm/spkgm/issues/1362
                        this._logger.warn(
                            'extra-resolution',
