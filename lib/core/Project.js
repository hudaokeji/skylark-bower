...
Project.prototype.install = function(decEndpoints, options, config) {
    ...
    return this._analyse()
        .spread(function(json, tree) {
            // It shows an error when issuing `spkgm install`
            // and no spkgm.json is present in current directory
            if (!that._jsonFile && decEndpoints.length === 0) {
                throw createError('No spkgm.json present', 'ENOENT');
            }
...
Project.prototype.saveJson = function(forceCreate) {
    ...
    if (!this._jsonFile && !forceCreate) {
        this._logger.warn(
            'no-json',
            'No spkgm.json file to save to, use spkgm init to create one'
        );
        return Q.resolve();
    }

    file = this._jsonFile || path.join(this._config.cwd, 'spkgm.json');
        ...
        .spread(function(json, deprecated, assumed) {
            ...
            if (!assumed) {
                that._jsonFile = path.join(
                    that._config.cwd,
                    deprecated ? deprecated : 'spkgm.json'
                );
            }
...
Project.prototype._readInstalled = function() {
    ...
    return (this._installed = Q.nfcall(glob, '*/.spkgm.json', {
        cwd: componentsDir,
        dot: true
