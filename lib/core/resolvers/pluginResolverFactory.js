...
function pluginResolverFactory(resolverFactory, spkgm) {
    spkgm = spkgm || {};
    ...
    var resolver = resolverFactory(spkgm);
    ...
    PluginResolver.prototype._savePkgMeta = function(meta, result) {
        ...
        return Q.nfcall(
            fs.writeFile,
            path.join(this._tempDir, '.spkgm.json'),
            contents
        ).then(function() {
    ...
    // It is used only by "spkgm info". It returns all semver versions.
    ...
    PluginResolver.clearRuntimeCache = function() {
        resolver = resolverFactory(spkgm);
    };
