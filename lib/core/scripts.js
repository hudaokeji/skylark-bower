...
var orderByDependencies = function(packages, installed, json) {
    ...
    //get the list of the packages that are specified in spkgm.json in that order
    //its nice to maintain that order for users
    var desiredOrder = mout.array.intersection(depsFromBowerJson, packageNames);
    //then add to the end any remaining packages that werent in spkgm.json
...
var run = function(cmdString, action, logger, config) {
    ...
    //pass env + BOWER_PID so callees can identify a preinstall+postinstall from the same spkgm instance
