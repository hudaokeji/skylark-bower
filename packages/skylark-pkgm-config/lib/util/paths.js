...
if (process.platform === 'win32') {
    ...
    paths.config = paths.config || path.join(base, '.config/spkgm');
    paths.data = paths.data || path.join(base, '.local/share/spkgm');
    paths.cache = paths.cache || path.join(base, '.cache/spkgm');
}

paths.tmp = path.resolve(path.join(tmp, 'spkgm'));
