...
Similar to `decompose()` but specially designed to be used when parsing `spkgm.json` dependencies.
For instance, in a `spkgm.json` like this:
...
Takes a decomposed endpoint and composes it to be saved to `spkgm.json`.
